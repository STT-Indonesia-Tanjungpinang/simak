    ...
    switch ($this->getDriver()) {
      case 'excel2003':               
        case 'excel2007':                        
          ...
          $sheet->setCellValue('A1', 'Nomor Transkrip / Transcript Number : ' . $biodata['dataTranskrip']['nomor_transkrip']);	
          $styleArray=array(					
            'font' => array('bold' => true),			                 
          );
          $sheet->getStyle("A1:A1")->applyFromArray($styleArray);  
          $sheet->getRowDimension(2)->setRowHeight(192.5);
          ...
          $row+=1;
          $sheet->mergeCells("A$row:M$row");        
          $sheet->setCellValue("A$row","ACADEMIC TRANSCRIPT");                                
          ...
          $sheet->getStyle("A4:A4")->applyFromArray($styleArray);           

          $row+=2;            
          $sheet->setCellValue("A$row","NIM / Student Registration Number");                                        
          $sheet->setCellValue("C$row", ": ".$biodata['nim']);                                        
          $sheet->setCellValue("H$row","Program Studi / Major");                                        
          $sheet->setCellValue("J$row", ": ".$biodata['nama_ps']);                                        
          
          $row+=1;        
          $sheet->setCellValue("A$row","NAMA / Name");                                        
          $sheet->setCellValue("C$row", ": ".$biodata['nama_mhs']);     
          $sheet->setCellValue("H$row","Program Pendidikan");                                        
          $sheet->setCellValue("J$row", ": Sarjana (S1)");                                                              
          ...
          $row+=1;        
          $sheet->setCellValue("A$row","Nomor Seri Ijazah / Diploma Number");                                        
          $sheet->setCellValueExplicit("C$row", ': '.$biodata['dataTranskrip']['nomor_ijazah'],PHPExcel_Cell_DataType::TYPE_STRING);          
          $sheet->setCellValue("H$row","Gelar / Degree");                                        
          $sheet->setCellValue("J$row", ": Sarjana Komputer (S.Kom.)");                                                                                            
          ...
          $sheet->getStyle("A".($row - 1).":F$row")->applyFromArray($styleArray);
          $sheet->getStyle("H".($row - 1).":M$row")->applyFromArray($styleArray);
          $sheet->getStyle("A".($row - 1).":M$row")->getAlignment()->setWrapText(true);
          ...
          $sheet->getStyle("H$row_awal:M$row")->applyFromArray($styleArray);
          ...
          $sheet->setCellValue("D$row", ": " . $biodata['dataTranskrip']['judul_skripsi']);
          $sheet->getStyle("D$row:D$row")->getAlignment()->setWrapText(true); 
          ...
          $sheet->mergeCells("J$row:M$row");     
          $sheet->setCellValue("J$row", 'Ketua');                      
          $row += 1;
          $sheet->setCellValue("A$row", "Indeks Prestasi Kumulatif / Grade Point Academic (GPA) : " . $objNilai->getIPKAdaNilai()); 
          $sheet->mergeCells("J$row:M$row");     
          $sheet->setCellValue("J$row", 'Director');                      
          $row += 1;
          $sheet->setCellValue("A$row", "Predikat / Predicate : " . $biodata['dataTranskrip']['predikat_kelulusan']); 
          $row += 4;
          $sheet->setCellValue("J$row", "Aurora Elsa Shafira Frederick, S.E., M.B.A."); 
          $row += 1;
          $sheet->setCellValue("J$row", "NIDN. 1014089701"); 
          $row += 1;
          $sheet->setCellValue("J$row", "Asisten Ahli"); 
          ...
          $this->printOut("transkrip_final_$nim");
        break;
      case 'pdf':
        $rpt=$this->rpt;
