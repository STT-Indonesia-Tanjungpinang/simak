...
class CFormulirPendaftaran extends MainPageMB 
{
  ...
  private function editProcess() 
  {
    ...
    //marketing
    $this->cmbEditChannelMarketing->DataSource = $this->DMaster->getListChannelMarketing();
    $this->cmbEditChannelMarketing->Text = $dataMhs['id_channel_marketing'];    
    $this->cmbEditChannelMarketing->dataBind();
  ...
  {     
    $this->idProcess = 'add';        
    $no_formulir= $this->Pengguna->getDataUser('no_formulir');
    $str = "SELECT nama_mhs,tempat_lahir,tanggal_lahir,jk,email,telp_hp,kjur1,kjur2,idkelas,id_channel_marketing,nama_referensi FROM formulir_pendaftaran_temp WHERE no_formulir = $no_formulir";
    $this->DB->setFieldTable(array('nama_mhs', 'tempat_lahir', 'tanggal_lahir', 'jk', 'email', 'telp_hp', 'kjur1', 'kjur2', 'idkelas', 'id_channel_marketing', 'nama_referensi'));
    $r = $this->DB->getRecord($str);
    
    $this->txtAddNoFormulir->Text = $no_formulir;		
    
    $this->txtAddNamaMhs->Text = $r[1]['nama_mhs'];			
    $this->txtAddTempatLahir->Text = $r[1]['tempat_lahir'];						
    $this->txtAddTanggalLahir->Text = $this->TGL->tanggal('d-m-Y', $r[1]['tanggal_lahir'],'entoid');
     if ($r[1]['jk']=='L')
      $this->rdAddPria->Checked=true;
    else
      $this->rdAddWanita->Checked=true;
    ...
    //marketing
    $this->cmbAddChannelMarketing->DataSource = $this->DMaster->getListChannelMarketing();
    $this->cmbAddChannelMarketing->Text = $r[1]['id_channel_marketing'];    
    $this->cmbAddChannelMarketing->dataBind();
    $this->txtAddNamaReferensi->Text = $r[1]['nama_referensi'];
    ...
    $this->cmbAddKjur1->Text = $r[1]['kjur1'];
    $this->cmbAddKjur1->dataBind();
    if ($r[1]['kjur2'] > 0)
    {
      $jurusan = $this->DMaster->removeKjur($_SESSION['daftar_jurusan'], $r[1]['kjur2']);			
  ...
  public function saveData($sender, $param)
  {
    ...
    if ($this->IsValid)
    {
      ...
      //marketing      
      $id_channel_marketing = $this->cmbAddChannelMarketing->Text;
      $nama_referensi = trim($this->txtAddNamaReferensi->Text);
      ...
      $str = "INSERT INTO formulir_pendaftaran (no_formulir,nama_mhs,tempat_lahir,tanggal_lahir,jk,idagama,nama_ibu_kandung,idwarga,nik,idstatus,alamat_kantor,alamat_rumah,kelurahan,kecamatan,telp_kantor,telp_rumah,telp_hp,idjp,pendidikan_terakhir,jurusan,kota,provinsi,tahun_pa,jenis_slta,asal_slta,status_slta,nomor_ijazah,id_channel_marketing,nama_referensi,kjur1,kjur2,waktu_mendaftar,ta,idsmt,idkelas,daftar_via) VALUES ('$no_formulir', '$nama_mhs', '$tempat_lahir', '$tgl_lahir', '$jk', $idagama,'$nama_ibu_kandung', '$idwarga', '$no_ktp', '$idstatus', '$alamat_kantor', '$alamat_rumah', '$kelurahan', '$kecamatan', '$telp_kantor', '$telp_rumah', '$telp_hp', $idjp,'$pendidikan_terakhir', '$jurusan', '$kota', '$provinsi', '$tahun_pa', '$jenisslta', '$asal_slta', '$statusslta', '$nomor_ijazah', '$id_channel_marketing', '$nama_referensi', '$kjur1', '$kjur2', '$waktu_mendaftar', $ta, $idsmt,'$idkelas', 'WEB')";		
      $this->DB->query('BEGIN');
  ...
  public function updateData($sender, $param)
  {
    ...
    if ($this->IsValid)
    {
      ...
      //marketing      
      $id_channel_marketing = $this->cmbEditChannelMarketing->Text;
      $nama_referensi = trim($this->txtEditNamaReferensi->Text);
      ...
      $str = "UPDATE formulir_pendaftaran SET nama_mhs='$nama_mhs',tempat_lahir='$tempat_lahir',tanggal_lahir='$tgl_lahir',jk='$jk',idagama = $idagama,nama_ibu_kandung='$nama_ibu_kandung',idwarga='$idwarga',nik='$no_ktp',idstatus='$idstatus',alamat_kantor='$alamat_kantor',alamat_rumah='$alamat_rumah',kelurahan='$kelurahan',kecamatan='$kecamatan',telp_kantor='$telp_kantor',telp_rumah='$telp_rumah',telp_hp='$telp_hp',idjp = $idjp,pendidikan_terakhir='$pendidikan_terakhir',jurusan='$jurusan',kota = '$kota',provinsi='$provinsi',tahun_pa='$tahun_pa',jenis_slta = '$jenisslta',asal_slta = '$asal_slta',status_slta = '$statusslta',nomor_ijazah='$nomor_ijazah',id_channel_marketing='$id_channel_marketing', nama_referensi='$nama_referensi', kjur1='$kjur1',kjur2='$kjur2',waktu_mendaftar='$waktu_mendaftar',ta = $ta,idsmt=$idsmt,idkelas='$idkelas',daftar_via='WEB' WHERE no_formulir='$no_formulir'";
      $this->DB->query('BEGIN');
      if ($this->DB->updateRecord($str)) {
        $str  = "UPDATE formulir_pendaftaran_temp SET nama_mhs='$nama_mhs',tempat_lahir='$tempat_lahir',tanggal_lahir='$tgl_lahir',jk='$jk',email='$email',telp_hp='$telp_hp',id_channel_marketing='$id_channel_marketing', nama_referensi='$nama_referensi', kjur1='$kjur1',kjur2='$kjur2',idkelas='$idkelas' WHERE no_formulir = $no_formulir";
        $this->DB->updateRecord($str);
        
        $email = $this->txtEditEmail->Text; 
        $userpassword=md5($_SESSION['userpassword_mb']);				
        $str = "UPDATE profiles_mahasiswa SET userpassword='$userpassword',email='$email',photo_profile='$photo_profile' WHERE no_formulir = $no_formulir";
        $this->DB->updateRecord($str);
        $_SESSION['foto'] = $photo_profile;
        $this->DB->query('COMMIT');
      }else {
