...
class CUserCN extends MainPageSA
{		    	
  public function onLoad($param)
  {
    parent::onLoad($param);		     
    $this->showSubMenuSettingSistem = true;
    $this->showUserCN = true;   
    if (!$this->IsPostBack && !$this->IsCallback)
    {
      if (!isset($_SESSION['currentPageUserCN']) || $_SESSION['currentPageUserCN']['page_name'] != 'sa.settings.UserCN')
      {
        $_SESSION['currentPageUserCN'] = array('page_name' => 'sa.settings.UserCN', 'page_num' => 0, 'search' => false);
      }
      $_SESSION['currentPageUserCN']['search'] = false;
      $this->populateData();            
    }
  }       
  ...
  public function Page_Changed($sender, $param) {
    $_SESSION['currentPageUserCN']['page_num'] = $param->NewPageIndex;
    $this->populateData($_SESSION['currentPageUserCN']['search']);
  }
  
  public function searchRecord($sender, $param) {
    $_SESSION['currentPageUserCN']['search']=true;
    $this->populateData($_SESSION['currentPageUserCN']['search']);
  }    
  protected function populateData($search = false) {
    if ($search) {
      $str = "SELECT u.userid,u.username,u.nama,u.email,ug.group_name,u.active,u.foto,u.logintime FROM user u LEFT JOIN user_group ug ON (ug.group_id=u.group_id) WHERE page='cn'";			
      $txtsearch = $this->txtKriteria->Text;
      switch($this->cmbKriteria->Text) {
        case 'username':
          $clausa = "AND username='$txtsearch'";
          $jumlah_baris = $this->DB->getCountRowsOfTable("user WHERE page='cn' $clausa",'userid');		            
          $str = "$str $clausa";
        break;
        case 'nama':
      ...
      $jumlah_baris = $this->DB->getCountRowsOfTable("user WHERE page='cn'",'userid');		            
      $str = "SELECT u.userid,u.username,u.nama,u.email,ug.group_name,u.active,u.foto,u.kjur,u.logintime FROM user u LEFT JOIN user_group ug ON (ug.group_id=u.group_id) WHERE page='cn'";			
    }
    $this->RepeaterS->CurrentPageIndex = $_SESSION['currentPageUserCN']['page_num'];
    ...
    if ($limit < 0) {$offset=0;$limit = $this->setup->getSettingValue('default_pagesize');$_SESSION['currentPageUserCN']['page_num'] = 0;}
  ...
  public function saveData($sender, $param) {
    if ($this->Page->isValid) {
      ...
      $this->redirect('settings.UserCN', true);
    }
  }
