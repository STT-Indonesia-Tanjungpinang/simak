...
class CUserKeuangan extends MainPageSA {		    	
	...
	public function Page_Changed ($sender,$param) {
		$_SESSION['currentPageUserKeuangan']['page_num']=$param->NewPageIndex;
		$this->populateData($_SESSION['currentPageUserKeuangan']['search']);
	}
    
    public function searchRecord ($sender,$param) {
		$_SESSION['currentPageUserKeuangan']['search']=true;
        $this->populateData($_SESSION['currentPageUserKeuangan']['search']);
	}    
	protected function populateData ($search=false) {
        if ($search) {
            ...
            $jumlah_baris=$this->DB->getCountRowsOfTable("user WHERE page='k'",'userid');		            
            $str = "SELECT u.userid,u.username,u.nama,u.email,ug.group_name,u.active,u.foto,u.kjur,u.logintime FROM user u LEFT JOIN user_group ug ON (ug.group_id=u.group_id) WHERE page='k'";			
        }
        $this->RepeaterS->CurrentPageIndex=$_SESSION['currentPageUserKeuangan']['page_num'];
		...
		if ($limit < 0) {$offset=0;$limit=$this->setup->getSettingValue('default_pagesize');$_SESSION['currentPageUserKeuangan']['page_num']=0;}
