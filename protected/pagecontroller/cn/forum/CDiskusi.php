...
class CDiskusi extends MainPageCN {        
	public function onLoad($param) {		
		parent::onLoad($param);		            
        $this->showForumDiskusi=true;                     
        $this->createObj('forum');
		if (!$this->IsPostBack && !$this->IsCallback) {              
            if (!isset($_SESSION['currentPageDiskusi']) || $_SESSION['currentPageDiskusi']['page_name'] != 'cn.forum.Diskusi') {                                                                                
                $_SESSION['currentPageDiskusi'] = array('page_name' => 'cn.forum.Diskusi', 'page_num' => 0,'page_num_unread' =>0, 'search' => false,'activeviewindex' => 0);
            }
