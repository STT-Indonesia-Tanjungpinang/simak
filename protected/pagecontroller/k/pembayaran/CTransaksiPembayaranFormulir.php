...
class CTransaksiPembayaranFormulir Extends MainPageK {	
    ...
    public function populateData() {
        ...
        $str = "SELECT k.idkombi,k.nama_kombi,kpt.biaya FROM kombi_per_ta kpt,kombi k WHERE  k.idkombi=kpt.idkombi AND tahun = $ta AND kpt.idkelas='$kelas' AND idsmt=$idsmt AND k.idkombi=1 ORDER BY periode_pembayaran,nama_kombi ASC";
...
{   
	...
	{	
        $this->createObj('Finance');
        $writer->write($this->Finance->toRupiah(CTransaksiPembayaranFormulir::$TotalKomponenBiaya));	
	}
}
...
{   
	...
	{	
        $this->createObj('Finance');
        $writer->write($this->Finance->toRupiah(CTransaksiPembayaranFormulir::$TotalSudahDibayarkan));	
	}
}
