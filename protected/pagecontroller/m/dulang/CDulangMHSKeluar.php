...
class CDulangMHSKeluar Extends MainPageM {		
	public function onLoad($param) {
		...
		if (!$this->IsPostBack && !$this->IsCallback) {
            ...
            $this->tbCmbTahunMasuk->Text=$_SESSION['currentPageDulangMHSKeluar']['tahun_masuk'];						
    ...
    public function setInfoToolbar() {        
		...
		$tahunmasuk=$_SESSION['currentPageDulangMHSKeluar']['tahun_masuk'] == 'none'?'':'Tahun Masuk '.$this->DMaster->getNamaTA($_SESSION['currentPageDulangMHSKeluar']['tahun_masuk']);		        
		$this->lblModulHeader->Text="Program Studi $ps T.A $ta Semester $semester $tahunmasuk";        
	}
    public function Page_Changed ($sender,$param) {
		$_SESSION['currentPageDulangMHSKeluar']['page_num']=$param->NewPageIndex;
		$this->populateData($_SESSION['currentPageDulangMHSKeluar']['search']);
	}
	...
	public function changeTbTA ($sender,$param) {				
		...
		$_SESSION['currentPageDulangMHSKeluar']['tahun_masuk']=$_SESSION['ta'];
	...
	public function changeTbTahunMasuk($sender,$param) {				
		$_SESSION['currentPageDulangMHSKeluar']['tahun_masuk']=$this->tbCmbTahunMasuk->Text;
        $this->setInfoToolbar();
		$this->populateData();
	}
    ...
    public function searchRecord ($sender,$param){
        $_SESSION['currentPageDulangMHSKeluar']['search']=true;
        $this->populateData($_SESSION['currentPageDulangMHSKeluar']['search']);
    }
    public function populateData($search=false) {
		...
		$kjur=$_SESSION['kjur'];
		$tahun_masuk=$_SESSION['currentPageDulangMHSKeluar']['tahun_masuk'];
        $iddosen_wali=$_SESSION['currentPageDulangMHSKeluar']['iddosen_wali'];
        ...
        if ($search) {
            $str = "SELECT d.iddulang,vdm.no_formulir,vdm.nim,vdm.nirm,vdm.nama_mhs,vdm.iddosen_wali,d.tanggal,d.tahun,d.idsmt,d.idkelas FROM v_datamhs vdm,dulang d WHERE vdm.nim=d.nim AND d.k_status='K'";
            $txtsearch=addslashes($this->txtKriteria->Text);
            switch ($this->cmbKriteria->Text) {
                case 'no_formulir' :
                    $clausa="AND vdm.no_formulir='$txtsearch'";
                    $jumlah_baris=$this->DB->getCountRowsOfTable ("v_datamhs vdm,dulang d WHERE vdm.nim=d.nim AND d.k_status='K' $clausa",'vdm.nim');
                    $str = "$str $clausa";
                break;
                case 'nim' :
            ...
            $str = "SELECT d.iddulang,vdm.no_formulir,vdm.nim,vdm.nirm,vdm.nama_mhs,vdm.iddosen_wali,d.tanggal,d.tahun,d.idsmt,d.idkelas FROM v_datamhs vdm,dulang d WHERE vdm.nim=d.nim AND d.tahun=$ta AND d.idsmt=$idsmt AND vdm.kjur='$kjur' AND d.k_status='K' $str_dw $str_tahun_masuk";
            $jumlah_baris=$this->DB->getCountRowsOfTable ("v_datamhs vdm,dulang d WHERE vdm.nim=d.nim AND d.tahun=$ta AND d.idsmt=$idsmt AND vdm.kjur='$kjur' AND d.k_status='K' $str_dw $str_tahun_masuk",'vdm.nim');
        }
		
		$this->RepeaterS->CurrentPageIndex=$_SESSION['currentPageDulangMHSKeluar']['page_num'];
		...
		if ($limit < 0) {$offset=0;$limit=10;$_SESSION['currentPageDulangMHSKeluar']['page_num']=0;}
        ...
        if ($nim != '') {
            try {
                if (!isset($_SESSION['currentPageDulangMHSKeluar']['DataMHS']['no_formulir'])) {                    
                    ...
                    $_SESSION['currentPageDulangMHSKeluar']['DataMHS']=$datamhs;
                }
            }catch (Exception $e) {
                $param->IsValid=false;
    ...
    public function printOut ($sender,$param) {		
        ...
        $this->lblPrintout->Text='Daftar Ulang Mahasiswa KELUAR';
